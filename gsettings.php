...
<div class="navispace">
	<div class="naviname">Settings Panel: <?php echo $_GET['gname']; ?></div>
</div>		
			<div class="facelook">
				<?php
				$gname=$_GET['gname'];
		echo	"<form method='GET' action='sposts.php'>
					<input type='hidden' name='uid' value='$uname'>
					<input type='hidden' name='gname' value='$gname'>
					<input type='submit' name='spostsSubmit' value='Posts Settings'>
					</form>";
					?>

				<div class='gulist'>User, Posts
				<?php
				guserList($gconn);
				?>
				</div>
				<br /><br />
				<div class='bgimage'>
				<p>Insert the group image</p>
				<?php

				$uid = $_GET['uid'];
				$gname = $_GET['gname'];
	echo		"<form method='POST' enctype='multipart/form-data' action='".photo($gconn)."'>
					<input type='hidden' name='uid' value='$uid'>
					<input type='hidden' name='gname' value='$gname'>
					<input type='file' id='photofile' name='photofile' accept='image/*'>
						<label for='photofile'>Choose the Picture</label>
					<input type='submit' name='photoSubmit' value='Insert'>
				</form>";
				?>
				</div>
			</div>
