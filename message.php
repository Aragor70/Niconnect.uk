...
<div class="navispace">
	<div class="naviname">Messager: <?php echo $_GET['listuname']."<br />"; 
?></div> 
</div>
			<div class="box">
			<?php
			...
			?>
				<div class="omessager">	
				<input type="hidden" id='listuname' value='<?php echo $_GET['listuname'];?>'>
...
echo	"<div class='sending'>";
				$uadres = $_GET['listuname'];
echo		"<form method='post' enctype='multipart/form-data' action='".setChat($conn)."'>
					
					<input type='hidden' name='uid' value='$uname' >
					<input type='hidden' name='adres' value='$uadres' >
					<input type='hidden' name='date' value='".date('Y-m-d h-i-s')."' >
					<input type='text' name='chatmessage' placeholder=' .Your message' >
					<input type='submit' name='chatSubmit' value='Throw' />
				</div>";
				?>
			</div>
				<script>
					var listuname = document.getElementById("listuname").value;
					var http = new XMLHttpRequest();
					http.onreadystatechange=function(){
						if(this.readyState==4 && this.status==200){
							var userchat = document.getElementById("userchat").innerHTML=this.responseText;
						}
					};
					http.open("GET", "userChat.php?listuname="+listuname, true);
					http.send();
		...
		<div class="three">
				...
				<div class="profile">
					<button id='profileWindowButton' style='color: #303030; text-align: justify;'><?php echo $uname."'s profile"; ?></button>
