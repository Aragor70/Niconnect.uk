...
<div class="navispace">
	<div class="naviname">The Group Page</div>
</div>		
			<div class="groupContainer">
			
			<div class="ulistGroups" id="listGroups">
			...
			<p id='ulistGroups'>
				...
				<script>
					...
					http.onreadystatechange=function(){
						if(this.readyState==4 && this.status==200){
							var ulistGroups= document.getElementById("ulistGroups").innerHTML=this.responseText;
						}
					};
					http.open("GET", "ulistGroups.php", true);
					http.send();
				</script>
			
			<div class="listaGroups" id="listaGroups">
			Find the New One from the list<br />
			<?php
echo "<p>";
				listaGroups($gconn);
echo "</p>";			
			...
			<div class="adminGroups" id="adminGroups">
			Your created groups <br />	
				<?php				
echo "<p>";
				adminGroups($gconn);
echo "</p>";
			...
			Create The New Group <br />
				<?php
					echo 	"<form method='POST' action='".setGroup($gconn)."'>
								<input type='hidden' name='uid' value='$uname'>
								<input type='hidden' name='date' value='".date('Y-m-d h:i:s')."'>
								<input type='text' name='gname' placeholder='Full name' style='cursor: text;'>
								<input type='submit' name='groupSubmit' value='Create'>
								</form>";
				?>
				</div>
				<div class="error">
				<?php
					if(isset($_SESSION['e_gname']))
						{
						echo $_SESSION['e_gname'];
						unset($_SESSION['e_gname']);
						}				
				?>
				</div>
			</div>
			...
			<script>
				...
				function hidder(){
					...
					//gmenu
					var listaGroups = document.getElementById("listaGroups");
					listaGroups.style.width="35%";
					var addGroup = document.getElementById("addGroup");
					addGroup.style.width="35%";
					
					var adminGroups =document.getElementById("adminGroups");
					adminGroups.style.width="60%";
					var listGroups =document.getElementById("listGroups");
					listGroups.style.width = "60%";
