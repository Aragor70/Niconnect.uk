...
<div class="navispace">
	<div class="naviname">Your own Pocket <?php echo $uname; ?></div>
</div>
...
<div class="facelook">
					...
					include "note.php";
								$uname = $_SESSION['user'];
								echo $uname;
					echo	"<form method='POST' action='".setArt($conn)."'>
								<input type='hidden' name='uid' value='$uname'>
								<input type='hidden' name='date' value='".date('Y-m-d H:i:s')."'>
								<input type='text' name='tittle' placeholder=' .The Tittle' />
								<textarea name='note' rows='5' placeholder=' .Here'></textarea><br>
								<input type='submit' name='noteSubmit' value='Publish' />
								</form>";
					?>
					</div>
						<input type="hidden" id="pocket" value='<?php echo $_GET['pocket'];?>'>
					<div class="pockets" id="pockets">
				...
				<script>
					...
					http.onreadystatechange=function(){
						if(this.readyState==4 && this.status==200){
							var pockets = document.getElementById("pockets").innerHTML=this.responseText;
						}
					};
					http.open("GET", "getPocket.php?pocket="+pocket, true);
					http.send();
		...
		<div class="three">
			...
			echo					"<form method='get' action='newProfile.php'>
									...
									<div class="pocketProfile">
										<form method='get' action='epocket.php'><input type='hidden' name='pocket' value='<?php echo $_SESSION['user']; ?>'><input type='submit' id='pocketProfileSubmit' name='pocketPageSubmit' value='Pocket' /></form>
									</div>
